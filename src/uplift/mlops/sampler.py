...
class Undersampler(BaseSampler):
    ...
    def get_splits(self, indices, data):
        ...
        if result is None:
            ...
            print("==================== loading splits: undersampling ============================")
        return indices

    def _generate_splits(self, indices, data):
        print("==================== generating splits: undersampling ============================")
        if type(data) != PolarsData:
            raise NotImplementedError(f"Undersampler not implemented for data type {type(data)}")
        ...
        for fold_idx, data_idx in indices.items():
            ...
            prob, m, major_class, minor_class = self._compute_sampling_prob(value_counts, data.metadata['target_name'])
            # compute subset of indices
            subindices = self._subsample_indices(idx, data, major_class, minor_class, count=m)
    ...
    def _subsample_indices(self, idx, data, major, minor, count=None, p=None):
        if p is not None:
            raise NotImplementedError("Subsampling with probability not implemented for Undersampler.")
        if count is not None:
            indices_maj = data.data.filter(
                (pl.col(data.metadata['target_name']) == major)).sample(n=count, shuffle=True, seed=100).select("row_idx").to_numpy()
            indices_min = data.data.filter(
                (pl.col(data.metadata['target_name']) == minor)).select("row_idx").to_numpy()
            indices = np.concatenate([indices_maj, indices_min], axis=0)
                #pl.col("row_idx").is_in(idx)
            return indices
        else:
            raise ValueError("count or p needs to be not None for method _subsample_indices")
