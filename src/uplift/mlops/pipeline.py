...
class PipelineProto(BasePipeline):
    def __init__(self, params):
        ...
        self.splitter = SplitterFactory().create(self.params.splitter.name)(**self.params.splitter.parameters)
        if 'sampler' in self.params:
            self.sampler = SamplerFactory().create(self.params.sampler.name)(**self.params.sampler.parameters)
        self.indices = None
