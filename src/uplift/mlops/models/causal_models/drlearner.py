...
class MyDRLearner(CausalModel):
    def __init__(self, model_effect, effect_parameters, model_propensity, propensity_parameters, model_final, final_parameters, parameters):
        model_effect = SubmodelFactory().create(model_effect)(**effect_parameters)
        model_propensity = SubmodelFactory().create(model_propensity)(**propensity_parameters)
